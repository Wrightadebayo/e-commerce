...
(queryClient)=>async ({params}) => {
  const 
  response = await queryClient.ensureQueryData(
...
return (
  <section>
    ...
    <div className='mt-6 grid gap-y-8 lg:grid-cols-2
     ...
     <div>
      <h1 className=
      'capitalize text-3xl font-bold'>{title}</h1>
      <h4 className=
      'text-xl text-neutral-content font-bold mt-2'>{company}</h4>
     <p className=
     'mt-3 text-xl'>{dollarsAmount}</p>
     <p className=
     'mt-3 leading-8'>{description}</p>
     {/* COLORS */}
     <div className='mt-6'>
